...
var seed_service: SeedService = SeedService.new()
...
func run_once(campaign_seed: String = DEFAULT_CAMPAIGN_SEED, run_index: int = 1, reset: bool = true) -> String:
    if reset:
        seed_service.init_campaign(campaign_seed)
    ...
    for entry in SCENARIO:
        ...
        var loot_rng := seed_service.prng_for_loot(realm_id, stage_index, encounter_index)
        ...
        var combat_rng := seed_service.prng_for_combat(realm_id, stage_index, encounter_index, "initiative")
        var init_rolls: Array[String] = []
        for i in range(10):
            init_rolls.append("%.6f" % combat_rng.next_float())

        var encounter_rng := seed_service.prng_for_encounter(realm_id, stage_index, encounter_index)
...
func snapshot_and_store() -> Dictionary:
    _cached_snapshot = seed_service.snapshot_state()
    return _cached_snapshot

func restore_cached_snapshot() -> void:
    if _cached_snapshot.is_empty():
        return
    seed_service.restore_state(_cached_snapshot)
